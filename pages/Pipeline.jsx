...
function usePersistentFilters() {
  const [filters, setFilters] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem('pipeline_filters') || '{}') }
    catch { return {} }
  })
  function setFilter(k, v) {
    setFilters(f => {
      const next = { ...f, [k]: v || undefined }
      if (!v) delete next[k]
      sessionStorage.setItem('pipeline_filters', JSON.stringify(next))
      return next
    })
  }
...
export default function Pipeline() {
  ...
  const [filters, setFilter] = usePersistentFilters()
  const [newOpen, setNewOpen] = useState(false)
  const [showExtended, setShowExtended] = useState(false)

  // Active stages shown in main board
  const activeStages = showExtended ? ALL_STAGES : STAGES
  ...
  async function moveStage(clientId, direction) {
    ...
    const next = ALL_STAGES[idx + direction]
    if (!next) return
  ...
  const metrics = {
    total:   clients.filter(c => STAGES.includes(c.stage)).length,
    high:    clients.filter(c => c.priority === 'High' && STAGES.includes(c.stage)).length,
    wip:     clients.filter(c => c.stage === 'WIP').length,
    ongoing: clients.filter(c => c.stage === 'Ongoing Management').length,
  }

  // Restore filter UI values from persisted state
  const searchVal   = filters.search || ''
  const typeVal     = filters.entity_type || ''
  const priorityVal = filters.priority || ''
  const execVal     = filters.client_executive || ''
  const dirVal      = filters.proposed_director || ''

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Header */}
      <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          />
          <select value={typeVal} onChange={e => setFilter('entity_type', e.target.value)}>
            <option value="">All types</option>
            <option>Trust</option><option>Company</option><option>Both</option>
          </select>
          <select value={priorityVal} onChange={e => setFilter('priority', e.target.value)}>
            <option value="">All priority</option>
            <option>High</option><option>Medium</option><option>Low</option>
          </select>
          <select value={execVal} onChange={e => setFilter('client_executive', e.target.value)}>
            <option value="">All executives</option>
            <option value="James Rambo">James Rambo</option>
            <option value="Michael Salgado">Michael Salgado</option>
            <option value="Mitchell">Mitchell</option>
          </select>
          <select value={dirVal} onChange={e => setFilter('proposed_director', e.target.value)}>
            <option value="">All directors</option>
            <option value="Andrew Fox">Andrew Fox</option>
            <option value="William Du Toit">William Du Toit</option>
          </select>
          <button className="btn btn-ghost" onClick={() => navigate('/advisors/investment')}>+ Inv. Advisor</button>
          <button className="btn btn-ghost" onClick={() => navigate('/advisors/tax')}>+ Tax Advisor</button>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--surface)' }}>
        {[
          { label: 'Active clients',     val: metrics.total },
          { label: 'High priority',      val: metrics.high },
          { label: 'WIP',                val: metrics.wip },
          { label: 'Ongoing Management', val: metrics.ongoing },
        ].map((m, i) => (
          <div key={i} style={{ flex: 1, padding: '10px 1.25rem', borderRight: '1px solid var(--border)' }}>
            <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 2 }}>{m.label}</div>
            <div style={{ fontSize: 20, fontWeight: 600 }}>{m.val}</div>
          </div>
        ))}
        {/* Toggle Long Term / Lost */}
        <div style={{ padding: '10px 1.25rem', display: 'flex', alignItems: 'center' }}>
          <button
            onClick={() => setShowExtended(s => !s)}
            style={{
              fontSize: 12, padding: '5px 12px', borderRadius: 6,
              border: '1px solid var(--border)',
              background: showExtended ? '#F5F3FF' : 'transparent',
              color: showExtended ? '#5B21B6' : 'var(--text-3)',
              cursor: 'pointer', whiteSpace: 'nowrap'
            }}
          >
            {showExtended ? '▼ Hide' : '▶ Show'} Long Term & Lost
            {!showExtended && (
              <span style={{ marginLeft: 6, background: '#E5E7EB', borderRadius: 10, padding: '1px 6px', fontSize: 11, color: '#374151' }}>
                {clients.filter(c => ['Long Term','Lost'].includes(c.stage)).length}
              </span>
            )}
          </button>
        </div>
      ...
      <div style={{
        ...
        {activeStages.map(stage => {
          ...
          const isExtended = stage === 'Long Term' || stage === 'Lost'
          return (
            <div key={stage} style={{ minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{
                ...
                opacity: isExtended ? 0.85 : 1,
              }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: m.text }}>{stage}</span>
                <span style={{ fontSize: 11, fontWeight: 600, background: m.color, color: '#fff', borderRadius: 20, padding: '1px 7px' }}>{cols.length}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {loading && <div style={{ textAlign: 'center', padding: 20, color: 'var(--text-3)', fontSize: 12 }}>…</div>}
                {!loading && cols.length === 0 && (
                  <div style={{ fontSize: 12, color: 'var(--text-3)', textAlign: 'center', padding: '12px 0' }}>No clients</div>
                )}
                {cols.map(client => (
                  <ClientCard
                    key={client.id}
                    ...
                    stageIndex={ALL_STAGES.indexOf(stage)}
                  />
                ))}
              </div>
            </div>
      ...
      <NewClientModal open={newOpen} onClose={() => setNewOpen(false)} onCreated={refetch} />
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 3 }}>
        ...
        {(client.ubo_name || client.proposed_entity_name) && (
          <MetaRow icon="🪪" label="UBO" value={client.ubo_name || client.proposed_entity_name} />
        )}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 3 }}>
        ...
        <div style={{ display: 'flex', gap: 2 }} onClick={e => e.stopPropagation()}>
          ...
          {stageIndex < ALL_STAGES.length - 1 && <button onClick={() => onMove(1)} style={{ fontSize: 11, color: 'var(--accent)', padding: '1px 4px', fontWeight: 600 }}>→</button>}
        </div>
      </div>
    </div>
  )
